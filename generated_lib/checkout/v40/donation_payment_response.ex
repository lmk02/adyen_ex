defmodule AdyenEx.Checkout.V40.DonationPaymentResponse do
  @moduledoc """
  Provides struct and type for a DonationPaymentResponse
  """

  @type t :: %__MODULE__{
          amount: AdyenEx.Checkout.V40.Amount.t() | nil,
          donationAccount: String.t() | nil,
          id: String.t() | nil,
          merchantAccount: String.t() | nil,
          payment: AdyenEx.Checkout.V40.PaymentResponse.t() | nil,
          reference: String.t() | nil,
          status: String.t() | nil
        }

  defstruct [:amount, :donationAccount, :id, :merchantAccount, :payment, :reference, :status]

  @doc false
  @spec __fields__(atom) :: keyword
  def __fields__(type \\ :t)

  def __fields__(:t) do
    [
      amount: {AdyenEx.Checkout.V40.Amount, :t},
      donationAccount: :string,
      id: :string,
      merchantAccount: :string,
      payment: {AdyenEx.Checkout.V40.PaymentResponse, :t},
      reference: :string,
      status: {:enum, ["completed", "pending", "refused"]}
    ]
  end
end
