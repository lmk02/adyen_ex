defmodule AdyenEx.Checkout.V71.AuthenticationData do
  @moduledoc """
  Provides struct and type for a AuthenticationData
  """

  @type t :: %__MODULE__{
          attemptAuthentication: String.t() | nil,
          authenticationOnly: boolean | nil,
          threeDSRequestData: AdyenEx.Checkout.V71.ThreeDSRequestData.t() | nil
        }

  defstruct [:attemptAuthentication, :authenticationOnly, :threeDSRequestData]

  @doc false
  @spec __fields__(atom) :: keyword
  def __fields__(type \\ :t)

  def __fields__(:t) do
    [
      attemptAuthentication: {:enum, ["always", "never"]},
      authenticationOnly: :boolean,
      threeDSRequestData: {AdyenEx.Checkout.V71.ThreeDSRequestData, :t}
    ]
  end
end
